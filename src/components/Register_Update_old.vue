...
export default {
  ...
  methods: {
      ...
      if(this.updateProfile === true){
        ...
        for(index=0; index < this.previousPhotosFiles.length; index++){
            ...
            if(this.previousPhotosFiles[index].name === file[0].name){
                console.log("this.previousPhotosFiles - Before", JSON.stringify(this.previousPhotosFiles));
                this.$refs.photo.removeFile(file[0]);
                this.previousPhotosFiles.splice(index,1);
                index--;
                uploadFile = false;
                console.log("this.previousPhotosFiles - After", JSON.stringify(this.previousPhotosFiles));
            } //end-if
          //} //end -for
        } //end-for
    ...
    async uploadProof (file) {
      ...
      if(this.updateProfile === true){
        ...
        for(index =0; index < this.previousProofFile.length; index++ ){
            //console.log("Upload Proof File Details", file[0].name, pFile.name)
        ...
        for (photo of photos) {
          ...
          axios({
            ...
            .then(response => {
              ...
              console.log("Photos Loaded", fileName, fileObj, this.previousPhotosFiles);
              this.showProgressBar = false;
    ...
    async updateForm () {
      ...
      console.log('Update Form', this.formData, this.$refs, this.$refs.personalForm)
    ...
    deleteFile(fileName){
      ...
      axios({
          url: process.env.API + '/photos/' + this.userDetailsId + '/' + fileName,
          method: 'DELETE',
        })
          .then(response => {
          ...
          .catch(error => {
            console.log("Error in Detete", error);
            let errMsg = ''
            if ('message' in error.response.data) {
              // errMsg = error.response.data.error + " - " + error.response.data.message;
